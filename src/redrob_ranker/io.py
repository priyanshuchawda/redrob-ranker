...
def _audit_verdict(scored: ScoredCandidate) -> tuple[str, str, str]:
    ...
    strong_evidence = components.retrieval + components.ranking + components.evaluation >= 26
    ...
    if risk_hits or not strong_evidence:
        return "C", "Weak top-100 confidence due to evidence or risk flags.", "; ".join(risk_hits or ["review evidence depth"])
